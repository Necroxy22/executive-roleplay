...
DisplaySalary(playerid)
{
	...
	if(rows) 
	{
		...
		new title[48];
		format(title, sizeof(title), "Pendapatan: %s", pData[playerid][pName]);
...
DisplayPaycheck(playerid)
{
	...
	if(rows) 
	{
		...
		pajak = total / 100 * 10;
		hasil = total - pajak;
		format(list, sizeof(list), "%s"WHITE_E"     \tPajak:\t"RED_E"%s\n", list, FormatMoney(pajak));
		format(list, sizeof(list), "%s"WHITE_E"     \tTotal:\t"LG_E"%s\n", list, FormatMoney(hasil));
		new title[48];
		format(title, sizeof(title), "Paycheck: %s", pData[playerid][pName]);
		ShowPlayerDialog(playerid, DIALOG_PAYCHECK, DIALOG_STYLE_TABLIST_HEADERS, title, list, "Ambil", "Tutup");
	}
	else
	{
		...
		format(list, sizeof(list), "%s"WHITE_E"     \tTotal:\t"LG_E"%s\n", list, FormatMoney(hasil));
		new title[48];
		format(title, sizeof(title), "Gaji: %s", pData[playerid][pName]);
		ShowPlayerDialog(playerid, DIALOG_PAYCHECK, DIALOG_STYLE_TABLIST_HEADERS, title, list, "Ambil", "Tutup");
	}
	return 1;
}
